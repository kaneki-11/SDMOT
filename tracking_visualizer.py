...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QPen, QColor, QFont, QTextCursor, QIcon
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 第一部分：序列操作（水平布局）
        seq_group = QGroupBox("序列操作")
        seq_layout = QHBoxLayout()
        seq_layout.setSpacing(10)

        # 加载序列按钮
        self.load_seq_btn = QPushButton("加载序列")
        self.load_seq_btn.setIcon(QIcon(":/icons/folder.png"))
        self.load_seq_btn.setMinimumWidth(100)
        self.load_seq_btn.setStyleSheet("QPushButton { padding: 5px 10px; }")
        self.load_seq_btn.clicked.connect(self.load_sequences)

        # 序列选择
        self.seq_combo = QComboBox()
        self.seq_combo.setMinimumWidth(250)
        self.seq_combo.setSizeAdjustPolicy(QComboBox.AdjustToContents)
        self.seq_combo.setStyleSheet("QComboBox { padding: 5px; }")
        ...
        self.process_btn.setStyleSheet(
            "QPushButton { padding: 5px 10px; background-color: #4CAF50; color: white; }")
        self.process_btn.setEnabled(False)
        self.process_btn.clicked.connect(self.process_sequence)

        # 可视化按钮
        self.visualize_btn = QPushButton("可视化跟踪结果")
        self.visualize_btn.setIcon(QIcon(":/icons/eye.png"))
        self.visualize_btn.setMinimumWidth(140)
        self.visualize_btn.setStyleSheet(
            "QPushButton { padding: 5px 10px; background-color: #2196F3; color: white; }")
        self.visualize_btn.setEnabled(False)
        self.visualize_btn.clicked.connect(self.visualize_tracks)

        # 添加到序列操作布局
        seq_layout.addWidget(self.load_seq_btn)
        seq_layout.addWidget(QLabel("选择序列:"))
        seq_layout.addWidget(self.seq_combo, 1)
        seq_layout.addWidget(self.process_btn)
        seq_layout.addWidget(self.visualize_btn)
        seq_layout.addStretch(1)

        seq_group.setLayout(seq_layout)
        control_layout.addWidget(seq_group)
        ...
        # 尾迹长度
        param_layout.addWidget(QLabel("尾迹长度:"), 0, 0)
        self.trail_spin = QSpinBox()
        self.trail_spin.setRange(1, 100)
        self.trail_spin.setValue(30)
        self.trail_spin.setMinimumWidth(80)
        self.trail_spin.setStyleSheet("QSpinBox { padding: 5px; }")
        param_layout.addWidget(self.trail_spin, 0, 1)

        # FPS
        param_layout.addWidget(QLabel("FPS:"), 0, 2)
        self.fps_spin = QDoubleSpinBox()
        self.fps_spin.setRange(1, 60)
        self.fps_spin.setValue(25.0)
        self.fps_spin.setMinimumWidth(80)
        self.fps_spin.setStyleSheet("QDoubleSpinBox { padding: 5px; }")
        param_layout.addWidget(self.fps_spin, 0, 3)

        # YOLO模型
        param_layout.addWidget(QLabel("YOLO模型:"), 1, 0)
        self.yolo_combo = QComboBox()
        self.yolo_combo.setMinimumWidth(200)
        self.yolo_combo.setStyleSheet("QComboBox { padding: 5px; }")
        # 从配置中加载默认模型
        default_yolo = self.tracking_system.config['yolo_model']
        self.yolo_combo.addItem(os.path.basename(default_yolo), default_yolo)
        self.yolo_combo.addItem("dance_yolo11s_best.pt", str(Path(default_yolo).parent / "dance_yolo11s_best.pt"))
        param_layout.addWidget(self.yolo_combo, 1, 1)
        ...
        # 跟踪方法
        param_layout.addWidget(QLabel("跟踪方法:"), 1, 2)
        self.track_method_combo = QComboBox()
        track_methods = ["bytetrack", "bytetrack_with_ekf_improv", "bytetrack_with_ukf_improv", "bytetrack_with_ekf"]
        ...
        self.track_method_combo.setMinimumWidth(150)
        self.track_method_combo.setStyleSheet("QComboBox { padding: 5px; }")
        param_layout.addWidget(self.track_method_combo, 1, 3)
        ...
        # self.status_label.setStyleSheet("QLabel { font-style: italic; color: #555; }")
    ...
    def process_sequence(self):
        ...
        # 新增：更新配置为选择的模型和方法
        self.tracking_system.config['yolo_model'] = self.yolo_combo.currentData()
        self.tracking_system.config['tracking_method'] = self.track_method_combo.currentText()
