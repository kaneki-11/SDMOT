...
def generate_dets_embs(args: argparse.Namespace, y: Path, source: Path) -> None:
    ...
    for frame_idx, r in enumerate(tqdm(results, desc="Frames")):
        ...
        for reid, reid_model_name in zip(reids, args.reid_model):
            ...
            embs_path = args.project / "dets_n_embs" / y.stem / 'embs' / reid_model_name.stem / (
...
def generate_mot_results(args: argparse.Namespace, config_dict: dict = None) -> None:
    ...
    print(f"Tracking results will be saved to: {txt_path}")
    all_mot_results = []
    ...
    for frame_idx, d in enumerate(tqdm(dataset, desc="Frames")):
        ...
        if tracks.size > 0:
            ...
            print(f"Frame {frame_idx + 1}: Generated {len(mot_results)} tracks")
        else:
            print(f"Frame {frame_idx + 1}: No tracks generated")
...
def trackeval(args: argparse.Namespace, seq_paths: list, save_dir: Path, MOT_results_folder: Path, gt_folder: Path,
              metrics: list = ["HOTA", "CLEAR", "Identity"]) -> str:
    """
...
def run_generate_dets_embs(opt: argparse.Namespace) -> None:
    ...
    # 过滤掉隐藏文件夹
    mot_folder_paths = [item for item in Path(opt.source).iterdir() if item.is_dir() and not item.name.startswith('.')]
    ...
    for y in opt.yolo_model:
        for i, mot_folder_path in enumerate(mot_folder_paths):
            ...
            embs_path = Path(opt.project) / 'dets_n_embs' / y.stem / 'embs' / (opt.reid_model[0].stem) / (
            ...
            if dets_path.exists() and embs_path.exists():
                if prompt_overwrite('Detections and Embeddings', dets_path, opt.ci):
                    ...
                    continue
            LOGGER.info(
                f'Generating detections and embeddings for data under {mot_folder_path} [{i + 1}/{len(mot_folder_paths)} seqs]')
            # generate_dets_embs(opt, y, source=mot_folder_path / 'img1')
            generate_dets_embs(opt, y, source=mot_folder_path)
...
def run_generate_mot_results(opt: argparse.Namespace, evolve_config: dict = None) -> None:
    ...
    """
    for y in opt.yolo_model:
        exp_folder_path = opt.project / 'mot' / (
                    str(y.stem) + "_" + str(opt.reid_model[0].stem) + "_" + str(opt.tracking_method))
        LOGGER.info(f"Experiment folder path: {exp_folder_path}")
        exp_folder_path = increment_path(path=exp_folder_path, sep="_", exist_ok=False)
        opt.exp_folder_path = exp_folder_path
        LOGGER.info(f"Incremented experiment folder path: {exp_folder_path}")

        # 确保目录被创建
        exp_folder_path.mkdir(parents=True, exist_ok=True)
        LOGGER.info(f"Created directory: {exp_folder_path}")

        # mot_folder_names = [item.stem for item in Path(opt.source).iterdir()]
        mot_folder_names = [Path(opt.source).name]  # 例如 ['v_iF9bKPWdZlc_c001']
        dets_file_paths = [item for item in (opt.project / "dets_n_embs" / y.stem / 'dets').glob('*.txt')
                           if not item.name.startswith('.')
                           and item.stem in mot_folder_names]
        embs_file_paths = [item for item in
                           (opt.project / "dets_n_embs" / y.stem / 'embs' / opt.reid_model[0].stem).glob('*.txt')
                           if not item.name.startswith('.')
                           and item.stem in mot_folder_names]

        print(f"mot_folder_names: {mot_folder_names}")
        print(f"dets_file_paths: {dets_file_paths}")
        print(f"embs_file_paths: {embs_file_paths}")
...
def run_all(opt: argparse.Namespace) -> None:
    ...
    print("Detections and embeddings generated")
    run_generate_mot_results(opt)
    print("MOT results generated")
    # run_trackeval(opt)
...
def parse_opt() -> argparse.Namespace:
    ...
    # parser.add_argument('--reid-model', nargs='+', type=Path, default=None)
    ...
    parser.add_argument('--objectives', type=str, nargs='+', default=["HOTA", "MOTA", "IDF1"],
                        help='set of objective metrics: HOTA,MOTA,IDF1')
    parser.add_argument('--val-tools-path', type=Path, default=EXAMPLES / 'val_utils',
                        help='path to store trackeval repo in')
    ...
    generate_dets_embs_parser.add_argument('--source', type=str, required=True, help='file/dir/URL/ 0 for webcam')
    generate_dets_embs_parser.add_argument('--yolo-model', nargs='+', type=Path, default=WEIGHTS / 'yolov8n.pt',
                                           help='yolo model path')
    generate_dets_embs_parser.add_argument('--reid-model', nargs='+', type=Path,
                                           default=WEIGHTS / 'osnet_x0_25_msmt17.pt', help='reid model path')
    generate_dets_embs_parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640],
                                           help='inference size h,w')
    generate_dets_embs_parser.add_argument('--classes', nargs='+', type=int, default=0,
                                           help='filter by class: --classes 0, or --classes 0 2 3')
    ...
    generate_mot_results_parser.add_argument('--yolo-model', nargs='+', type=Path, default=WEIGHTS / 'yolov8n.pt',
                                             help='yolo model path')
    generate_mot_results_parser.add_argument('--reid-model', nargs='+', type=Path,
                                             default=WEIGHTS / 'osnet_x0_25_msmt17.pt', help='reid model path')
    generate_mot_results_parser.add_argument('--tracking-method', type=str, default='deepocsort',
                                             help='deepocsort, botsort, strongsort, ocsort, bytetrack, imprassoc')
    generate_mot_results_parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640],
                                             help='inference size h,w')
